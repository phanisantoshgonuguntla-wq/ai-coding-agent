...
def test_list_codegen_sessions_routes_to_handler():
    assert agent_commands.run_agent("list codegen sessions", make_context()) == "sessions"


def test_show_codegen_session_routes_session_id_to_handler():
    assert agent_commands.run_agent(
        "show codegen session codegen_123",
        make_context(),
    ) == "session:codegen_123"
